...
void setup()   
{
  ...
  irsend.begin();
  gpio_set_drive_capability((gpio_num_t)IRLED, (gpio_drive_cap_t)GPIO_DRIVE_CAP_3);
  pinMode(LED, OUTPUT);
...
void sendAllCodes() 
{
  ...
  // Thêm độ trễ 100ms trước khi bật lại IR Jammer
  delay(100);
...
void loop() 
{
  ...
  if (buttonPressed) {
    ...
    if (isTransmitting) {
      ...
      // Thêm độ trễ 300ms trước khi chuyển từ IR Jammer sang TV-B-Gone
      delay(100);
      jammerEnabled = false;
      sendAllCodes();        
    }
